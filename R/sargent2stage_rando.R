...
sargent2stage_R <- function(p0, pa, alpha, beta, eta, pi, eps = 0.005, N_min, N_max, alloc, ...) { # eps=tolerance limit
  ...
  res0_t <- data.frame(do.call("rbind", mapply(function(a, b, c) cbind(N = a, rtemp = (-c):(b - 2), N_E = b, N_C = c), # mapply outputs a list of matrices
    ...
    SIMPLIFY = F
  ))) # list elements (one N-all r's) to one matrix
  ...
  res0 <- aggregate(res0_t[res0_t$beta <= (beta + eps), ]$rtemp, by = list(
    res0_t[res0_t$beta <= (beta + eps), c("N")],
    res0_t[res0_t$beta <= (beta + eps), c("N_E")],
    res0_t[res0_t$beta <= (beta + eps), c("N_C")]
  ), FUN = max)
  ...
  for (i in 1:length(res3_list)) {
    ...
    if (i == 1) {
      res3_list_new <- list(dt)
    } # Create list with first dataset
    if (i != 1) {
      res3_list_new[[i]] <- dt
    } # Next iterations: append dataset
  }
  close(pb)
  ...
  for (i in 1:length(res3_1_list)) {
    ...
    if (i == 1) {
      res3_1_list_new <- list(dt)
    } # Create list with first dataset
    if (i != 1) {
      res3_1_list_new[[i]] <- dt
    } # Next iterations: append dataset
  }
  close(pb)
  ...
  for (i in 1:length(res4_1_list)) {
    ...
    if (i == 1) {
      res4_1_list_new <- list(dt)
    } # Create list with first dataset
    if (i != 1) {
      res4_1_list_new[[i]] <- dt
    } # Next iterations: append dataset
  }
  close(pb)
